...
unsigned int nStakeMinAge = 60 * 60 * 12;	// minimum age for coin age: 12h
...
const string strMessageMagic = "Rekt Signed Message:\n";
...
static const int64 nTargetTimespan = 60 * 10;  // 10 blocks
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Rekt
    bool fStrictPayToScriptHash = true; // Always active in Rekt
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rekt", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "GET REKT";
        CTransaction txNew;
        txNew.nTime = 1414531864;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1414531864;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1308718;
